...
# mips64-elf-gcc via Homebrew tap
if command -v mips64-elf-gcc &>/dev/null; then
    ...
    if "$BREW" install tehzz/n64-dev/mips64-elf-gcc >> "$LOG" 2>&1 && command -v mips64-elf-gcc &>/dev/null; then
        ...
        warn "mips64-elf-gcc (brew install failed)"
    fi
fi
...
echo "  🐱 CAT'S TWEAKER v3.5.5 loaded!"
ENV
chmod +x "$INSTALL_DIR/env.sh"; ok "env.sh"
add_path ""; add_path "# Cat's Tweaker v3.5.5"; add_path "source \"$INSTALL_DIR/env.sh\" 2>/dev/null"
...
command -v mips64-elf-gcc &>/dev/null && ok "mips64-elf-gcc" || warn "mips64-elf-gcc"
[[ -f "$COMPILERS/devkitpro/devkitpro.pkg" ]] && ok "DevkitPro"

echo ""
printf "${G}╔═════════════════════════════════════════════════════════════════════╗${RST}\n"
printf "${G}║${RST}  ${W}✨ CAT'S TWEAKER v3.5.5 COMPLETE! ✨${RST}                             ${G}║${RST}\n"
