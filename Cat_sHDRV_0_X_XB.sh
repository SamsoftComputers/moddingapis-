...
#     ╚═════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝       ╚═╝    ╚══╝╚══╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
#
#                       「  v3.5.7 - M4 PRO FINAL EDITION  」
#                         1930-2026 + NES→PS5 TOOLKIT
#                            by Flames / Team Flames 🐱
#                 [PATCHED: DASM, z88dk, rgbasm, mips64-elf-gcc auto-install]
#                 [v3.5.7: libdragon Docker MIPS extraction for M4 Pro]
#
#===============================================================================
...
step "MIPS64 TOOLCHAIN (libdragon/Docker)"
# Native MIPS64 toolchain extraction from libdragon Docker image
# Works on M4 Pro via Rosetta 2 - no compilation needed
mkdir -p "$COMPILERS/mips-toolchain"
if [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]]; then
    ok "mips64-elf-gcc (cached)"
else
    # Check Docker is running
    if ! docker info &>/dev/null; then
        warn "Docker not running - start Docker.app first"
        info "After Docker starts: re-run this script or manually:"
        info "  docker pull ghcr.io/dragonminded/libdragon:latest"
    else
        info "Pulling libdragon image (contains MIPS64 toolchain)..."
        if docker pull ghcr.io/dragonminded/libdragon:latest >> "$LOG" 2>&1; then
            ok "libdragon image pulled"
            info "Extracting MIPS64 toolchain from container..."
            # Create container, extract toolchain, remove container
            CID=$(docker create ghcr.io/dragonminded/libdragon:latest 2>>"$LOG")
            if [[ -n "$CID" ]]; then
                # Extract the full N64 toolchain
                docker cp "$CID:/n64_toolchain/." "$COMPILERS/mips-toolchain" >> "$LOG" 2>&1
                docker rm "$CID" >> "$LOG" 2>&1
                # Verify extraction
                if [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]]; then
                    ok "mips64-elf-gcc extracted"
                    # M4 Pro / Rosetta 2 compatibility check
                    if $IS_M4 && $HAS_ROSETTA; then
                        info "M4 Pro + Rosetta 2: x86_64 binaries will run via translation"
                        # Test the binary actually works
                        if "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" --version &>/dev/null; then
                            ok "mips64-elf-gcc verified (Rosetta 2)"
                        else
                            warn "mips64-elf-gcc binary test failed"
                        fi
                    fi
                else
                    warn "mips64-elf-gcc extraction failed"
                    info "Try manual: docker cp \$(docker create ghcr.io/dragonminded/libdragon:latest):/n64_toolchain/. ~/retro-dev/compilers/mips-toolchain"
                fi
            else
                warn "Container creation failed"
            fi
        else
            warn "libdragon pull failed"
            info "Manual: docker pull ghcr.io/dragonminded/libdragon:latest"
        fi
    fi
fi
...
# z88dk - multiple install methods
info "z88dk..."
if command -v z88dk-z80asm &>/dev/null; then
    ok "z88dk (cached)"
elif "$BREW" list z88dk &>/dev/null; then
    ok "z88dk (Homebrew)"
else
    mkdir -p "$COMPILERS/z88dk"; cd "$COMPILERS/z88dk"
    Z88_OK=""
    
    # Method 1: Nightly macOS binary (best option)
    info "Trying nightly binary..."
    Z88_URL=$(curl -fsSL "http://nightly.z88dk.org/" 2>/dev/null | grep -oE 'z88dk-macos[^"]+\.zip' | head -1)
    if [[ -n "$Z88_URL" ]]; then
        if dl "http://nightly.z88dk.org/$Z88_URL" z88dk.zip; then
            unzip -qo z88dk.zip >> "$LOG" 2>&1
            rm -f z88dk.zip
            # Find extracted dir
            Z88_DIR=$(find . -maxdepth 1 -type d -name "z88dk*" | head -1)
            if [[ -n "$Z88_DIR" && -f "$Z88_DIR/bin/z88dk-z80asm" ]]; then
                # Move contents up
                mv "$Z88_DIR"/* . 2>/dev/null
                rmdir "$Z88_DIR" 2>/dev/null
    ...
    # Method 2: Homebrew (may fail on ARM)
    if [[ -z "$Z88_OK" ]]; then
        info "Trying Homebrew..."
        if GIT_TERMINAL_PROMPT=0 "$BREW" install z88dk >> "$LOG" 2>&1 && command -v z88dk-z80asm &>/dev/null; then
            ok "z88dk (Homebrew)"
            Z88_OK=1
        fi
    fi
    ...
    # Method 3: Docker
    if [[ -z "$Z88_OK" ]] && command -v docker &>/dev/null; then
        info "Trying Docker..."
        if docker pull z88dk/z88dk >> "$LOG" 2>&1; then
            # Create wrapper scripts
            mkdir -p "$COMPILERS/z88dk/bin"
            for tool in zcc z88dk-z80asm z88dk-sccz80 z88dk-appmake; do
                cat > "$COMPILERS/z88dk/bin/$tool" << WRAP
#!/bin/bash
docker run --rm -v "\$(pwd):/src" -w /src z88dk/z88dk $tool "\$@"
WRAP
                chmod +x "$COMPILERS/z88dk/bin/$tool"
            done
            ok "z88dk (Docker)"
            Z88_OK=1
        fi
    ...
    # Method 4: Source build script (last resort)
    if [[ -z "$Z88_OK" ]]; then
        warn "z88dk auto-install failed"
        cat > build_z88dk.sh << 'Z88'
#!/bin/bash
set -e
cd ~/retro-dev/compilers/z88dk
git clone --recursive https://github.com/z88dk/z88dk.git src 2>/dev/null || (cd src && git pull)
cd src
export BUILD_SDCC=1
...
Z88
        ...
        info "Run: bash ~/retro-dev/compilers/z88dk/build_z88dk.sh"
    fi
fi
...
step "N64 (1996)"
mkdir -p "$COMPILERS/n64" "$COMPILERS/mips-toolchain/bin"

# mips64-elf-gcc installation
if command -v mips64-elf-gcc &>/dev/null; then
    ok "mips64-elf-gcc (cached)"
elif [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]]; then
    ok "mips64-elf-gcc (local)"
else
    info "Installing mips64-elf-gcc..."
    MIPS_OK=""
    
    # Method 1: Homebrew tap
    "$BREW" tap tehzz/n64-dev >> "$LOG" 2>&1
    if "$BREW" install tehzz/n64-dev/mips64-elf-gcc >> "$LOG" 2>&1 && command -v mips64-elf-gcc &>/dev/null; then
        ok "mips64-elf-gcc (Homebrew)"
        MIPS_OK=1
    fi
    
    # Method 2: Docker extraction
    if [[ -z "$MIPS_OK" ]] && command -v docker &>/dev/null; then
        info "Trying Docker extraction..."
        CONTAINER_ID=$(docker create ghcr.io/dragonminded/libdragon:latest 2>/dev/null)
        if [[ -n "$CONTAINER_ID" ]]; then
            mkdir -p "$COMPILERS/mips-toolchain"
            docker cp "$CONTAINER_ID:/n64_toolchain/bin/." "$COMPILERS/mips-toolchain/bin/" 2>>"$LOG"
            docker cp "$CONTAINER_ID:/n64_toolchain/lib/." "$COMPILERS/mips-toolchain/lib/" 2>>"$LOG"
            docker cp "$CONTAINER_ID:/n64_toolchain/libexec/." "$COMPILERS/mips-toolchain/libexec/" 2>>"$LOG"
            docker cp "$CONTAINER_ID:/n64_toolchain/mips64-elf/." "$COMPILERS/mips-toolchain/mips64-elf/" 2>>"$LOG"
            docker rm "$CONTAINER_ID" >> "$LOG" 2>&1
            chmod +x "$COMPILERS/mips-toolchain/bin/"* 2>/dev/null
            [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]] && { ok "mips64-elf-gcc (Docker)"; MIPS_OK=1; }
        fi
    fi
    
    # Method 3: Source build
    if [[ -z "$MIPS_OK" ]]; then
        warn "mips64-elf-gcc auto-install failed"
        cd "$COMPILERS/mips-toolchain"
        cat > build_mips.sh << 'MIPS'
#!/bin/bash
set -e
echo "🔧 Building MIPS64 toolchain (30-60 min)..."
PREFIX="$HOME/retro-dev/compilers/mips-toolchain"
TARGET="mips64-elf"; J=$(sysctl -n hw.ncpu)
brew install gmp mpfr libmpc texinfo 2>/dev/null || true
mkdir -p "$PREFIX/src" && cd "$PREFIX/src"
echo "📦 [1/2] binutils..."
curl -fsSL "https://ftp.gnu.org/gnu/binutils/binutils-2.42.tar.xz" -o b.tar.xz
tar xf b.tar.xz && cd binutils-* && mkdir -p build && cd build
../configure --target=$TARGET --prefix=$PREFIX --with-cpu=vr4300 --disable-nls --disable-werror
make -j$J && make install && cd "$PREFIX/src"
echo "📦 [2/2] gcc..."
curl -fsSL "https://ftp.gnu.org/gnu/gcc/gcc-13.2.0/gcc-13.2.0.tar.xz" -o g.tar.xz
tar xf g.tar.xz && cd gcc-* && ./contrib/download_prerequisites
mkdir -p build && cd build
../configure --target=$TARGET --prefix=$PREFIX --with-arch=vr4300 --enable-languages=c,c++ \
    --disable-threads --disable-nls --without-headers --with-newlib --disable-libssp --disable-multilib
...
[[ -d "$RETRO_DEV/compilers/mips-toolchain/bin" ]] && export PATH="$RETRO_DEV/compilers/mips-toolchain/bin:$PATH"
[[ -d "$DEVKITARM/bin" ]] && export PATH="$DEVKITARM/bin:$PATH"
[[ -d "$GBDK/bin" ]] && export PATH="$GBDK/bin:$PATH"
# z88dk support (nightly binary, source build, or docker wrappers)
[[ -d "$RETRO_DEV/compilers/z88dk/bin" ]] && {
    export PATH="$RETRO_DEV/compilers/z88dk/bin:$PATH"
    [[ -d "$RETRO_DEV/compilers/z88dk/lib/config" ]] && export ZCCCFG="$RETRO_DEV/compilers/z88dk/lib/config"
}
...
echo "  🐱 CAT'S TWEAKER v3.5.7 loaded!"
ENV
chmod +x "$INSTALL_DIR/env.sh"; ok "env.sh"
add_path ""; add_path "# Cat's Tweaker v3.5.7"; add_path "source \"$INSTALL_DIR/env.sh\" 2>/dev/null"
...
command -v z88dk-z80asm &>/dev/null && ok "z88dk" || { [[ -x "$COMPILERS/z88dk/bin/z88dk-z80asm" ]] && ok "z88dk (local)" || { [[ -x "$COMPILERS/z88dk/src/bin/z88dk-z80asm" ]] && ok "z88dk (source)" || warn "z88dk"; }; }
...
command -v mips64-elf-gcc &>/dev/null && ok "mips64-elf-gcc" || { [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]] && ok "mips64-elf-gcc (local)" || warn "mips64-elf-gcc"; }
[[ -f "$COMPILERS/devkitpro/devkitpro.pkg" ]] && ok "DevkitPro"

echo ""
printf "${G}╔═════════════════════════════════════════════════════════════════════╗${RST}\n"
printf "${G}║${RST}  ${W}✨ CAT'S TWEAKER v3.5.7 COMPLETE! ✨${RST}                             ${G}║${RST}\n"
...
[[ -f "$COMPILERS/mips-toolchain/build_mips.sh" ]] && ! command -v mips64-elf-gcc &>/dev/null && ! [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]] && echo "  3. bash ~/retro-dev/compilers/mips-toolchain/build_mips.sh"
echo ""
