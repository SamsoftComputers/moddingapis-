...
if $IS_MAC; then
    ...
    # Install packages one by one to avoid one failure stopping all
    BREW_OK=0
    BREW_FAIL=0
    for pkg in gcc llvm cmake ninja meson sdl2 sdl2_image sdl2_mixer sdl2_ttf \
               libpng jpeg freetype zlib python nasm yasm wget curl p7zip \
               glew glfw boost raylib rgbds cc65 sdcc node; do
        if brew install "$pkg" >> "$LOG" 2>&1; then
            ((BREW_OK++))
        else
            ((BREW_FAIL++))
            echo "[BREW] Failed: $pkg" >> "$LOG"
        fi
    done
    
    # Try wla-dx separately (different tap or name)
    brew install wla-dx >> "$LOG" 2>&1 || brew install vhelin/wla-dx/wla-dx >> "$LOG" 2>&1 || true
    
    if [[ $BREW_FAIL -eq 0 ]]; then
        ok "Brew packages ($BREW_OK installed)"
    else
        ok "Brew packages ($BREW_OK installed, $BREW_FAIL skipped)"
    fi
else
...
# Method 1: with --break-system-packages (Python 3.11+)
if pip3 install --user --break-system-packages pygame pillow numpy pyyaml toml >> "$LOG" 2>&1; then
    PIP_OK=true
# Method 2: without --break-system-packages (older Python)
elif pip3 install --user pygame pillow numpy pyyaml toml >> "$LOG" 2>&1; then
    PIP_OK=true
# Method 3: try python3 -m pip
elif python3 -m pip install --user pygame pillow numpy pyyaml toml >> "$LOG" 2>&1; then
    PIP_OK=true
fi

if $PIP_OK; then
    # Try optional packages (don't fail if these don't work)
    pip3 install --user --break-system-packages ursina pysdl2 intelhex pyserial capstone >> "$LOG" 2>&1 || \
    pip3 install --user ursina pysdl2 intelhex pyserial capstone >> "$LOG" 2>&1 || true
    ok "Python packages"
else
    fail "Python packages"
    info "Try: pip3 install --user pygame pillow numpy"
fi
...
if $IS_MAC; then
    ...
    info "N64 toolchain for macOS uses Docker + libdragon CLI"
    ...
    if ! command -v docker >/dev/null 2>&1; then
        ...
        if ! docker info >/dev/null 2>&1; then
            info "Docker installed but not running - start Docker Desktop"
        fi
        ...
        if command -v npm >/dev/null 2>&1; then
            ...
            if npm install -g libdragon >> "$LOG" 2>&1; then
                ...
                info ""
                info "  ╔═══════════════════════════════════════════════════╗"
                info "  ║  N64 SETUP COMPLETE! Create your first project:   ║"
                info "  ║                                                   ║"
                info "  ║  mkdir my-n64-game && cd my-n64-game              ║"
                info "  ║  libdragon init                                   ║"
                info "  ║  libdragon make                                   ║"
                info "  ╚═══════════════════════════════════════════════════╝"
                info ""
                ...
                fail "libdragon CLI"
                info "Try: npm install -g libdragon"
            fi
        else
